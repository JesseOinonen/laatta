...
\midrule
Geometry fetch         & In progress & D-03, D-04 \\
\quad Index fetch      & In progress & --- \\
\quad Vertex fetch / shading & In progress & D-03, D-04 \\
...
to the vertex shader; control signals \texttt{start}, \texttt{desc\_addr},
\texttt{busy}, \texttt{done}. Scene dimensions are not ports --- they are read
...
\field{Status.} In progress (baseline: descriptor read plus uncached fetch).

\subsection{Geometry fetch: read engine, sequencer and clock gating}

\field{Read engine.} A generic AXI4 read FSM (\texttt{axi4\_read}) handles the
\texttt{AR}/\texttt{R} handshake for one burst at a time. It is reused unchanged
for all three read types (descriptor, index, vertex): the sequencer presents an
address, length, size and burst type with a valid strobe, and the engine drives
the address channel, streams the read beats, and raises a single-cycle
\texttt{rdone} pulse on the last beat (\texttt{RLAST}) or on an error response.
An accepted read address cannot be altered mid-burst, so the engine always runs
a transaction to completion before returning to idle; \texttt{rdone} is the
handoff that tells the sequencer new address values may be presented. Only one
transaction is outstanding at a time --- \texttt{ARID}/\texttt{RID} are wired
but unused, deferred until measurement shows read latency to be a bottleneck.

\field{Sequencer.} A separate process, on the ungated clock, decides the order
of reads --- descriptor first, then the index/vertex loop --- and generates
each address. It hardcodes only the command base (\texttt{CMD\_BASE}); every
other address comes from the unpacked descriptor. It advances only while the
read engine is idle, so it never changes the engine's inputs mid-transaction.

\field{Clock gating.} The read engine's clock is gated by a latch-based
integrated clock-gating cell (\texttt{cg}), enabled by \texttt{axi\_active}.
This leaves the engine unclocked while the GPU has not been started --- the
legitimate coarse-grained use of clock gating, at block level rather than per
register. \texttt{axi\_active} is derived from the held \texttt{start} level
from the control register and is sampled only while the engine is in its IDLE
state, so the gate enable can change only at a transaction boundary. It can
therefore never gate off mid-burst, which would freeze the AXI handshake and
deadlock the bus. This relies on \texttt{start} being a held level, not a pulse;
the assumption must be preserved if the control interface changes.

\field{Idle behaviour.} In IDLE the engine forces only the handshake qualifiers
\texttt{ARVALID} and \texttt{RREADY} to 0. The address-channel payload
(\texttt{ARADDR}, \texttt{ARLEN}, \texttt{ARSIZE}, \texttt{ARBURST}) is left at
its last value, since AXI treats it as don't-care while \texttt{ARVALID} is low.
Resetting it is unnecessary: clock gating freezes registers at their last
defined value rather than randomising them, and the asynchronous reset (D-10)
initialises them, so there is no stale-data hazard when the gated clock
restarts.

\field{Status.} Read engine, sequencer and clock gate in bring-up; index and
vertex address generation and vertex assembly in progress.

...
end fp_rsqrt_wrapper;

architecture RTL of fp_rsqrt_wrapper is
    signal valid_shift_r : std_logic_vector(10 downto 0);
...
begin
    ...
    fp_sqrt_inst : entity work.fp_sqrt_core
        port map (
            clk => clk,
            X   => a,
            R   => b
    ...
    --------------------------------
    -- fp_sqrt_core pipeline depth is 6 
    -- fp_div_core pipeline depth is 5
    -- so data is valid for recip after 11 clock cycles
    valid_process : process(clk, rst_n)
    begin
        if rst_n = '0' then
            valid_shift_r <= (others => '0');
        elsif rising_edge(clk) then
            valid_shift_r <= valid_shift_r(9 downto 0) & valid_in;
        end if;
    end process valid_process;

    valid_out <= valid_shift_r(10);
